...
export let voteSMS = (request: Request, response: Response) => {
    if (twilio.validateExpressRequest(request, twilioConfig.key, {url: twilioConfig.smsWebhook}) || twilioConfig.disableTwilioSigCheck) {
        ...
        VoteSource.findOne({ PhoneNumber: to }, function(err, vote: VoteSourceModel) {
            if (err) {
                console.log(err);
